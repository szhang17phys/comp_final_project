...
void projectFinal(){
    ...
    double valid_xyz[num] = {0.0};//valid steps of x y z---
...
//---fill the graph------
	...
	mg_xz->GetXaxis()->SetRangeUser(-0.2, 1.2);

        rout_yz[i] = new TGraph(num, &pos_y[i][0], &pos_z[i][0]);
        rout_yz[i]->GetXaxis()->SetTitle("y");
        rout_yz[i]->GetYaxis()->SetTitle("z");
	rout_yz[i]->GetXaxis()->SetRangeUser(-0.5, 0.5);
	rout_yz[i]->GetYaxis()->SetRangeUser(0.0, 1.0);
	...
	mg_yz->GetXaxis()->SetRangeUser(-0.2, 1.2);
    }
...
//---TFile------
    ...
    TH2F axis("axis","axis",1,-1.0,1.0,2,-0.1,1.1); 
    axis.Draw("axis");
    mg_xz->Draw("LP");//no "A" here---
    canv1->SaveAs("plot1.png");
    canv1->Write();

    TCanvas *canv2 = new TCanvas("", "", 700, 500);
    axis.Draw("axis");
    mg_yz->Draw("LP");
    canv2->SaveAs("plot2.png");
    canv2->Write();
